...
class Version extends Cross
{
    ...
    public function index(Request $request)
    {
        ...
        $db = new VersionModel();
        $field = 'id';
        $list = $db->order('id DESC')->limit(1)->page($page)->field($field, true)->select();
        if ($list) {
            return json(['code' => 200, 'msg' => '获取成功', 'data' => $list[0]]);
        } else
    ...
    public function getLatestVersion(Request $request)
    {
        $db = new VersionModel();
        $field = '*';
        $list = $db->order('id DESC')->limit(1)->field($field)->select();
        if ($list) {
            return json(['code' => 200, 'msg' => '获取成功', 'data' => $list[0]]);
        } else
            return json(['code' => 1, 'msg' => '没数据']);
